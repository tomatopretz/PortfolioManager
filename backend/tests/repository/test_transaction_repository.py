...
@patch('repository.transaction_repository.get_cursor')
...
@patch('repository.transaction_repository.get_cursor')
def test_list_by_portfolio_item_filters_correctly(mock_get_cursor):
    cursor = MagicMock()
...
@patch('repository.transaction_repository.get_cursor')
def test_delete_returns_false_when_no_row_matched(mock_get_cursor):
    cursor = MagicMock()
    cursor.rowcount = 0
    _mock_cursor(mock_get_cursor, cursor)
